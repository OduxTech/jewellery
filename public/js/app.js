$(document).ready(function() {
        ...
        $.ajax({
            ...
            success: function(result) {
                if (result.success == true) {
                    ...
                    if(typeof gold_rate_table !== 'undefined') {
                        gold_rate_table.ajax.reload();
                    }
                    var evt = new CustomEvent("gold_rateAdded", {detail: result.data});
                ...
                setTimeout(function() {
                    location.reload();
                }, 1000); // 1 second delay to let toastr show
    ...
    var brands_table = $('#gold_rate_table').DataTable({
        processing: true,
        serverSide: true,
        fixedHeader:false,
        ajax: '/goldrate',
    ...
    $(document).on('click', 'button.edit_gold_rate_button', function() {
    ...
    $(document).on('click', 'button.delete_gold_rate_button', function() {
        swal({
            title: LANG.sure,
            text: LANG.confirm_delete_gold_rate,
            icon: 'warning',
            buttons: true,
            dangerMode: true,
        }).then(willDelete => {
